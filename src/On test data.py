...
df = pd.read_csv(r"C:\Users\asifu\Desktop\train.csv")
...
# defining SARIMA
model = ARIMA(df_train["log_adj_close"], order=(11, 1, 1), seasonal_order=(11,1,1,15)).fit()  # order=(p, d, q)
print(model.summary())
...
model.add(tf.keras.layers.SimpleRNN(50, activation='tanh', input_shape=(n_steps_in, 1)))
...
df_test['RNN'] = scaler.inverse_transform(np.expand_dims(forecasted.flatten(), axis=1))
...
plt.plot(df_train["Adj Close"], label="Train Data")
